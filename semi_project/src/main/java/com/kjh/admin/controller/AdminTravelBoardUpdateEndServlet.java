...
/**
 * Servlet implementation class AdminTravelBoardUpdateEndServlet
 */
@WebServlet("/admin/travelboardupdateend.do")
public class AdminTravelBoardUpdateEndServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminTravelBoardUpdateEndServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(!ServletFileUpload.isMultipartContent(request)) {
			...
			int boardNo=Integer.parseInt(mr.getParameter("boardNoTarget"));
			...
			TravelBoard board=
				TravelBoard.builder().boardNo(boardNo).
				boardTitle(title).
				boardContent(content).
				thumbFilename(thumbFilename).
				build();
				
			int result=new TravelBoardService().updateTravelBoard(board, tagList);
